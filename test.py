...
def compute_min_utility(
    N: int, eta: float, mu: Dict[FrozenSet[int], List[Tuple[float, float]]], sharing_rules: Dict[FrozenSet[int], Dict[int, float]]
) -> float:
    """
            ...
            for v2 in singleton_payoffs:
                for V in grand_payoffs:
                    for alpha1 in alpha_values:
                        ...
                        sharing_rules = {frozenset({1}): {1: 1.0}, frozenset({2}): {2: 1.0}, frozenset({1, 2}): {1: alpha1, 2: alpha2}}
